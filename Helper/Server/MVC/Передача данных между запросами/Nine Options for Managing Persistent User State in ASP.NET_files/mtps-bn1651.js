...
MTPS.Controls.BindCollapsibleAreas =
function() {

    this.applyClick = function(e, a) {
        var expand = false;
        if (a == undefined || a == 99)
            expand = jQuery(this).next().is(':hidden')
        else expand = a;

        if (expand && jQuery(this).next().is(':hidden')) {
            jQuery(this).next().show();
            jQuery(this).find(":nth-child(1)").addClass('LibC_o').removeClass('LibC_c');
        }
        else if (!expand && !jQuery(this).next().is(':hidden')) {
            jQuery(this).next().hide();
            jQuery(this).find(":nth-child(1)").addClass('LibC_c').removeClass('LibC_o');
        }

    };


    jQuery("div.CollapseRegionLink").bind('click', this.applyClick);

}
MTPS.Controls.BindCollapsibleAreas();

MTPS.Controls.BindSidebySideCollapsibleArea =
function() {
    this.applyClick = function(e, a) {
        if (a == undefined)
            e.data.trigger("click", 99);

    };
    howmany = jQuery("div.CollapseRegionLink").size();
    half = howmany / 2;
    for (currentone = 0; half < howmany; currentone++, half++) {
        jQuery("div.CollapseRegionLink:eq(" + currentone + ")").bind('click', jQuery("div.CollapseRegionLink:eq(" + half + ")"), this.applyClick);
        jQuery("div.CollapseRegionLink:eq(" + half + ")").bind('click', jQuery("div.CollapseRegionLink:eq(" + currentone + ")"), this.applyClick);
    }
};
